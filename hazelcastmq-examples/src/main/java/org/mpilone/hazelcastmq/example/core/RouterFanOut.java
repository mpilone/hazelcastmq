...
/**
 * Example of using a fan-out routing strategy to route messages from a * single source channel to multiple target channels.
 *
 * @author mpilone
 */
public class RouterFanOut extends ExampleApp {

  private final static ILogger log = Logger.getLogger(RouterFanOut.class);

  public static void main(String[] args) throws Exception {
    RouterFanOut app = new RouterFanOut();
    app.runExample();
  }
  ...
  public void start() throws Exception {
    ...
    try (Broker broker = HazelcastMQ.newBroker(brokerConfig)) {
      ...
      try (RouterContext context = broker.createRouterContext();
        ...
        router.setRoutingStrategy(new FanOutRoutingStrategy());
      }
