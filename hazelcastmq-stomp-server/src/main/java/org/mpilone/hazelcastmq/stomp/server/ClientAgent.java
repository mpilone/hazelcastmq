...
import org.mpilone.hazelcastmq.stomp.server.ClientSubscription.MessageCallback;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * A Stomp client connection which handles processing all STOMP commands for the
 * client using frame input and output streams.
 * 
 * @author mpilone
 */
class ClientAgent {
  private final String DEFAULT_CONTEXT_TXN_ID = "hazelcastmq-stomp-server-default";
   ...
   */
  private HazelcastMQStompServer stomper;
   ...
   * @throws JMSException
   */
  public ClientAgent(Socket clientSocket, HazelcastMQStompServer stomper)
      throws IOException {
    ...
    HazelcastMQContext defaultContext = config.getHazelcastMQInstance()
        .createContext();
    defaultContext.start();
    transactions.put(DEFAULT_CONTEXT_TXN_ID, new ClientTransaction(
        DEFAULT_CONTEXT_TXN_ID, defaultContext));
  ...
  public void runClientLoop() {
    ...
    for (ClientSubscription subscription : subscriptions.values()) {
      ...
      subscription.getContext().close();
    }
    subscriptions.clear();

    // Close all transactions
    for (ClientTransaction transaction : transactions.values()) {
      transaction.getContext().rollback();
      transaction.getContext().close();
    }
    transactions.clear();
  ...
  private void onSubscribe(Frame frame) throws IOException {
    ...
    String destination = getRequiredHeader("destination", frame);
