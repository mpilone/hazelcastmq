...
//		txObject.setSavepointAllowed(isNestedTransactionAllowed());
    ...
    try {
      if (txObject.getHazelcastMQContextHolder() == null || txObject.
        ...
        if (log.isDebugEnabled()) {
          log.debug("Acquired HazelcastMQContext [" + newCon
              + "] for HazelcastMQ transaction");
        }
  ...
  protected void doCommit(DefaultTransactionStatus status) throws
    ...
    if (status.isDebug()) {
      log.debug("Committing HazelcastMQ transaction on HazelcastMQContext ["
          + con + "]");
    }
  ...
  protected void doRollback(DefaultTransactionStatus status) throws
    ...
    if (status.isDebug()) {
      log.debug("Rolling back HazelcastMQ transaction on HazelcastMQContext ["
          + con + "]");
    }
  ...
  protected void doCleanupAfterCompletion(Object transaction) {
    ...
    if (txObject.isNewHazelcastMQContextHolder()) {
      ...
      if (log.isDebugEnabled()) {
        log.debug("Releasing HazelcastMQContext [" + con
            + "] after transaction");
      }
